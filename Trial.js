...
var toul=document.getElementsByClassName('data');
...
const sendData=(sRate,count)=>{
  ...
  // swal("Success","","success");
    ...
    swal("Sorry","Answer Key Challenge has been closed","error");
 }
}
const successMsg=()=>{
  let toul=document.getElementsByClassName("data");
  let count=toul.length-1;
  for(let i=0;i<toul.length;i++){
    if(toul[i].className==="data success"){
      let sRate=0+i;
...
function setErrorMsg(input,errormsgs){
  ...
  toul.className="data error";
  small.innerText=errormsgs;
}
function setSuccessMsg(input){
  const toul=input.parentElement;
  toul.className="data success";
}
